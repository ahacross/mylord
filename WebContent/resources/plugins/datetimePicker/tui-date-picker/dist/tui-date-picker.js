...
/***/ (function(module, exports, __webpack_require__) {
	...
	DatePicker.createCalendar = function(wrapperElement, options) {
		DatePicker.createDomContainer(wrapperElement);
		//bom
	...
	DatePicker.createRangePicker = function(options) {
		var containerIds = [options.startpicker.container, options.endpicker.container];
		
		for(var i=0; i<2; i++) {
			DatePicker.createDomContainer(containerIds[i]);
		}
		//bom
	    return new DateRangePicker(options);
	};

DatePicker.createDomContainer = function(id) {
	if(typeof id === "string") {
		id = id.substring(1);
		if($("body").find("#pickerContainerArea").length === 0){
			$("body").append('<div id="pickerContainerArea"></div>');
		}

		if($("#"+id).length === 0){
			$("body").find("#pickerContainerArea").append('<div id="'+id+'" class="picker-container" style="margin-left: -1px;"></div>');
		}else{
			$("body").find("#pickerContainerArea").find("#"+id).empty();
		}
	}
}
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var DatePicker = snippet.defineClass(/** @lends DatePicker.prototype */{
	    ...
	    init: function(container, options) {
			...
			DatePicker.createDomContainer(container);
			//bom
	        /**
	    ...
	    _initTimePicker: function(opTimePicker) {
	        ...
	        this._timepicker.on('change', function(ev) {
	            ...
	            if (this._date) {
	                ...
	                this.setDate(prevDate.setHours(ev.hour, ev.minute, ev.second));
	            }
	        }, this);
	    },
	    ...
	    _syncFromInput: function(shouldRollback) {
	        ...
	        try {
	            ...
	            if (this.isSelectable(date)) {
	                if (this._timepicker) {
	                    this._timepicker.setTime(date.getHours(), date.getMinutes(), date.getSeconds());
	                }
	                this.setDate(date);
	            } else {
	                isFailed = true;
	    ...
	    _onClickDate: function(ev) {
	        ...
	        if (calendarType !== pickerType) {
	            ...
	            if (timepicker) {
	                newDate.setHours(timepicker.getHour(), timepicker.getMinute(), timepicker.getSecond());
	            } else if (prevDate) {
	    ...
	    open: function(e) {			
			...
			const inputOffset = this._datepickerInput._$input.offset();
			this._$container.css({ top:inputOffset.top + this._datepickerInput._$input[0].offsetHeight, left:inputOffset.left});
			//bom
	    ...
	    setDate: function(date) {
	        ...
	        if (shouldUpdate) {
	            ...
	            if (this._timepicker) {
	                this._timepicker.setTime(newDate.getHours(), newDate.getMinutes(), newDate.getSeconds());
	            }
	            this._syncToInput();
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var rFormableKeys = /\\?(yyyy|yy|mmmm|mmm|mm|m|dd|d|hh|h|ss|s|a)/gi;
	...
	var DateTimeFormatter = snippet.defineClass(/** @lends DateTimeFormatter.prototype */{
	    ...
	    parse: function(str) {
	        ...
	        return new Date(dateHash.year, dateHash.month - 1, dateHash.date, dateHash.hour, dateHash.minute, dateHash.second);
	    },
	    ...
	    format: function(dateObj) {
	        ...
	        replaceMap = {
				...
				m: minute,
				s:second,
				ss:dateUtil.prependLeadingZero(second),
